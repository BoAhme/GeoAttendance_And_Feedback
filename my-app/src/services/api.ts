...
import type { User, UserRole, Course, Department, Classroom } from '../types/index.ts';
...
export const adminApi = {
  ...
  async updateDepartment(id: string, payload: Partial<Department>): Promise<Department> {
    ...
    return { id: updated.id, ...updated.data() } as Department;
  },
  ...
  async updateClassroom(id: string, payload: Partial<Classroom>): Promise<Classroom> {
    ...
    await updateDoc(docRef, payload);
    const updated = await getDoc(docRef);
    return { id: updated.id, ...updated.data() } as Classroom;
  },
