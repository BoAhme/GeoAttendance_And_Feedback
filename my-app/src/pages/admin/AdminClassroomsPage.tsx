...
export function AdminClassroomsPage() {
  ...
  const handleSave = async () => {
    ...
    setSubmitting(true);
    try {
      await adminApi.updateClassroom(editing.id, { name, building, lat, lng, geofenceRadiusMeters: radius });
      setClassrooms((prev) => prev.map((c) => (c.id === editing.id ? { ...c, name, building, lat, lng, geofenceRadiusMeters: radius } : c)));
  ...
  const columns: Column<Classroom>[] = [
    ...
    { id: 'coords', header: 'GPS', accessor: (r) => `${r.lat.toFixed(4)}, ${r.lng.toFixed(4)}` },
    { id: 'radius', header: 'Geofence (m)', accessor: (r) => r.geofenceRadiusMeters },
    {
      id: 'actions',
      header: 'Actions',
      accessor: (r) => (
  ...
  return (
    <AppShell title="Classrooms">
      <DataTable columns={columns} data={classrooms} keyExtractor={(r) => r.id} />
      <Modal
        isOpen={editOpen}
      ...
      >
        <div className="space-y-4">
          ...
          <FormInput label="Building" value={building} onChange={(e) => setBuilding(e.target.value)} fullWidth />
          <FormMapPicker
            label="Location & geofence"
            center={{ lat, lng }}
            radiusMeters={radius}
            onCenterChange={(la, ln) => { setLat(la); setLng(ln); }}
